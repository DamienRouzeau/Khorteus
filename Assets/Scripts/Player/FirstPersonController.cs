    ...
    public class FirstPersonController : MonoBehaviour, Health
    {
        ...
        private void Update()
        {
            ...
            if (crafting)
            {
                ...
                {
                    if(hit.collider.CompareTag("craft"))
                    {
                        Crafting _crafting = hit.collider.gameObject.GetComponent<Crafting>();
                        _crafting.currentCraftTime += Time.deltaTime;
                        craftGauge.value = _crafting.currentCraftTime / _crafting.timeToCraft;
                        if(_crafting.currentCraftTime >= _crafting.timeToCraft)
                        {
                            // ----------------------- give turret -----------------------
                        }
                    }
        ...
        private void OnInteract(InputValue value)
        {
            ...
            if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hit, interactionDistance))
            {
                ...
                switch (hit.collider.tag)
                {
                    ...
                    case "craft":
                        blockMovement = true;
                        craftUI.SetActive(value.isPressed);
                        crafting = value.isPressed;
                        Crafting table = hit.collider.gameObject.GetComponent<Crafting>();
                        break;
